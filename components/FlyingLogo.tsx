...
const LogoMark = ({ size, svgRef }: { size: number; svgRef: React.RefObject<SVGSVGElement | null> }) => (
  <svg
    ref={svgRef}
    ...
    style={{ display: "block", filter: "drop-shadow(0 0 8px rgba(255,31,142,0.55))" }}
  >
...
const FlyingLogo = forwardRef<FlyingLogoHandle, FlyingLogoProps>(
  ({ targetRef }, ref) => {
    ...
    return (
      ...
      >
        <LogoMark size={HERO_SIZE} svgRef={svgRef} />
      </div>
    );
  }
);
